...
DSH Web 会话费用估算 + 当前官方 DeepSeek API Key 所属账户余额。v0.2.1 适配 Harness 0.1.5-alpha.1。
...
- 缺失或无法识别的模型默认按 V4.1 Flash 计入费用，并显示“按 Flash 估算”条数；有有效请求时间时沿用峰谷规则。已识别模型的历史计价逻辑不变。费用是官方参考价估算，不代表第三方服务实际账单。
