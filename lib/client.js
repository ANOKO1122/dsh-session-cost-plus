window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		function BalanceLine() {
			...
			return react_jsx_runtime.jsx(BalanceView, { state, error, busy, onRefresh: () => refresh(n => n + 1) });
		}

		/** Theme-aware display, separated from polling for regression tests. */
		function BalanceView({ state, error, busy, onRefresh }) {
			...
			return react_jsx_runtime.jsxs("div", {
				"data-dsh-cost-balance": "",
				style: { display: "flex", alignItems: "center", justifyContent: "center", flexWrap: "wrap", gap: "2px 10px", color: "var(--dsw-alias-label-tertiary)", fontFamily: "inherit", fontSize: 12, lineHeight: "20px", padding: "2px calc(var(--dsh-composer-side-clearance, 0px) + 16px) 4px", whiteSpace: "normal", fontVariantNumeric: "tabular-nums" },
				title: "当前官方 DeepSeek API Key 所属账户余额，不是本会话余额或剩余 token 数；每30秒刷新，服务端最多缓存15秒",
				children: [
					state ? state.balances.map(b => react_jsx_runtime.jsxs("span", {
						style: { display: "inline-flex", alignItems: "baseline", flexWrap: "wrap", justifyContent: "center", gap: "2px 8px" },
						children: [
							react_jsx_runtime.jsx("span", { children: "官方余额" }),
							react_jsx_runtime.jsx("span", { style: { color: secondary, fontWeight: 500 }, children: symbol(b.currency) + b.total_balance }),
							react_jsx_runtime.jsx("span", { children: `充值 ${symbol(b.currency)}${b.topped_up_balance} · 赠送 ${symbol(b.currency)}${b.granted_balance}` }),
						],
					}, b.currency)) : react_jsx_runtime.jsx("span", { children: error ? "官方余额暂不可用" : "官方余额查询中…" }),
					state && react_jsx_runtime.jsx("span", { children: `${new Date(state.updatedAt).toLocaleTimeString()} 更新${!state.isAvailable ? " · 账户不可用" : ""}` }),
					error && react_jsx_runtime.jsx("span", { role: "status", style: { color: secondary, overflowWrap: "anywhere" }, children: `${state ? "旧数据 · 刷新失败：" : ""}${error}` }),
					react_jsx_runtime.jsx(Button, { variant: "ghost", size: "sm", disabled: busy, onClick: onRefresh, icon: refreshIcon, title: "查询官方账户余额（服务端最多缓存15秒）", "aria-label": "刷新官方账户余额", "aria-busy": busy, style: { color: secondary, fontFamily: "inherit" }, children: busy ? "刷新中" : "刷新" }),
				],
			});
		}
